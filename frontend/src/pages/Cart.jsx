  ...
  if (cartItems.length === 0) {
    return (
      <div className="pt-60 text-center px-6">
        ...
        <p className="text-gray-500 text-[10px] uppercase tracking-widest mb-10 font-bold">Hãy nhặt vài con mô hình xịn xò vào đây nhé!</p>
        ...
        <div className="lg:col-span-2 space-y-4">
          {cartItems.map(item => (
            <div key={item.id} className="bg-[#161616] p-6 rounded-[32px] border border-white/5 flex flex-col sm:flex-row items-center gap-6 group hover:border-white/10 transition-all">
              <img src={imageUrl(item.imageUrl)} className="w-24 h-24 rounded-2xl object-cover" alt={item.name} />
              ...
              <div className="flex-1 text-center sm:text-left">
                ...
                <p className="text-[10px] text-gray-600 mt-1">Tối đa: {item.stock} cái</p>
              </div>
              ...
              <div className="flex items-center bg-black rounded-xl p-1 border border-white/5">
                ...
                <button
                  onClick={() => {
                    ...
                    if (!ok) toast.error(`Chỉ còn ${item.stock} sản phẩm trong kho!`);
                  }}
                  className="w-8 h-8 flex items-center justify-center font-bold hover:text-orange-500"
                >
                  +
              ...
              <button onClick={() => removeFromCart(item.id)} className="text-gray-600 hover:text-red-500 transition p-2">
                🗑️
              </button>
            </div>
          ))}
